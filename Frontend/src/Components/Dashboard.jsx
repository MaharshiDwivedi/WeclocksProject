...
const Dashboard = () => {
  const [values, setValues] = useState({ actualExpense: 0, expectedExpense: 0 });
  ...
  const formatNumber = (value) => {
    if (value === 0) {
      return "0 Lakh"; // Special case for 0
        ...
        {headwiseData.map((head, index) => (
          <div key={index} className="p-4 border rounded-lg shadow-md">
            ...
            <BarGraph 
              width="250px" 
              height="200px" 
              actualExpense={head.actual_cost} 
              expectedExpense={head.expected_cost} 
            />
          </div>
        ))}
      </div>
