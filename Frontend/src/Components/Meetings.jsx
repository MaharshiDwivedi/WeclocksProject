...
import { X, Camera, Upload, CalendarPlus, Pencil, Trash2, ChevronRight } from "lucide-react"
...
const Meetings = () => {
  ...
  const handleDeleteMeeting = async (meetingId, event) => {
    ...
    if (window.confirm(t("confirmDeleteMeeting"))) {
  ...
  return (
    <div className="min-h-screen p-3 md:p-6 space-y-4 md:space-y-6">
      <h2 className="text-2xl md:text-4xl font-bold text-center text-blue-950 realfont2">{t("smcMeetings")}</h2>

      <div className="flex justify-start">
        <button
          onClick={toggleModal}
          className="flex items-center gap-1 md:gap-2 bg-blue-950 text-white px-2 py-1.5 md:py-2 rounded-md hover:bg-blue-900 transition-colors text-sm md:text-base"
        >
          <CalendarPlus className="w-5 h-5 md:w-9 md:h-9" />
          <span classnam>{t("addMeeting")}</span>
        </button>
      </div>

      {isOpen && (
        <div className="fixed z-10 inset-0 flex items-center justify-center bg-transparent backdrop-blur-[2px] h-full realfont p-2 md:p-0 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-md shadow-blue-950 w-full max-w-md h-[500px] md:h-[500px] flex flex-col">
            ...
            <div className="p-3 md:p-4 space-y-3 md:space-y-4 flex-1 overflow-y-auto">
              <div className="space-y-1 md:space-y-2">
                <label className="block text-xs md:text-sm font-medium text-blue-950">{t("date")}</label>
                <DatePicker
                  selected={selectedDate}
                  onChange={handleDateChange}
                  dateFormat="MMMM d, yyyy"
                  className="w-full px-2 md:px-3 py-1.5 md:py-2 shadow-lg rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              ...
              <div className="space-y-1 md:space-y-2">
                <label className="block text-xs md:text-sm font-medium text-blue-950">{t("selectedMembers")}</label>
                <div className="flex flex-wrap gap-1 md:gap-2">
                  {selectedMembers.map((member, index) => (
                    <span
                      key={index}
                      className="flex items-center gap-1 bg-purple-700 text-white px-2 md:px-3 py-0.5 md:py-1 rounded-full text-xs md:text-sm"
                    >
                      {member.name}
                      <button onClick={() => removeMember(member)} className="hover:text-gray-200 transition-colors">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  ))}
              ...
              <div className="space-y-1 md:space-y-2">
                <label className="block text-xs md:text-sm font-medium text-blue-950">
                  {t("committeeMembersLabel")}
                </label>
                <div className="shadow-lg rounded-md h-24 md:h-32 overflow-y-auto">
                  {committeeMembers.map((member, index) => (
                    <div
                      key={index}
                      onClick={() => handleMemberChange(member)}
                      className="flex items-center justify-between p-1.5 md:p-2 hover:bg-gray-100 cursor-pointer"
                    >
                      <div>
                        <p className="font-medium text-xs md:text-sm">{member.name}</p>
                        <p className="text-xs text-gray-600">{member.representative}</p>
                        <p className="text-xs text-gray-600">{member.designation}</p>
                      </div>
                      {selectedMembers.find((m) => m.id === member.id) && <span className="text-purple-700">✓</span>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-4">
                <div className="space-y-1 md:space-y-2">
                  <label className="block text-xs md:text-sm font-medium text-blue-950">{t("latitude")}</label>
                  <div className="p-1.5 md:p-2 shadow-lg rounded-md bg-gray-50 text-center text-xs md:text-sm">
                    {loading ? (
                      <div className="w-4 h-4 md:w-5 md:h-5 border-2 border-blue-950 border-t-transparent rounded-full animate-spin mx-auto" />
                    ) : (
                      latitude
                    )}
                  </div>
                ...
                <div className="space-y-1 md:space-y-2">
                  <label className="block text-xs md:text-sm font-medium text-blue-950">{t("longitude")}</label>
                  <div className="p-1.5 md:p-2 shadow-lg rounded-md bg-gray-50 text-center text-xs md:text-sm">
                    {loading ? (
                      <div className="w-4 h-4 md:w-5 md:h-5 border-2 border-blue-950 border-t-transparent rounded-full animate-spin mx-auto" />
                    ) : (
              ...
              <div className="space-y-1 md:space-y-2">
                <label className="block text-xs md:text-sm font-medium text-blue-950">{t("address")}</label>
                <input
                  value={address}
                  readOnly
                  className="w-full px-2 md:px-3 py-1.5 md:py-2 shadow-lg rounded-md bg-gray-50 text-xs md:text-sm"
                />
              </div>

              <div className="space-y-1 md:space-y-2">
                <label className="block text-xs md:text-sm font-medium text-blue-950">{t("uploadOrTakePhoto")}</label>
                <div className="flex gap-2 md:gap-4">
                  <button
                    onClick={() => document.getElementById("fileInput").click()}
                    className="flex-1 flex items-center justify-center gap-1 md:gap-2 bg-green-600 text-white px-2 md:px-4 py-1.5 md:py-2 rounded-md hover:bg-green-700 transition-colors text-xs md:text-sm"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 md:gap-2 bg-purple-600 text-white px-2 md:px-4 py-1.5 md:py-2 rounded-md hover:bg-purple-700 transition-colors text-xs md:text-sm"
                  >
                ...
                {photo && !showCamera && (
                  <div className="mt-2 shadow-lg rounded-md overflow-hidden">
                    <img src={photo || "/placeholder.svg"} alt={t("selectedPhoto")} className="w-full" />
                ...
                {showCamera && (
                  <div className="mt-2 md:mt-4 border rounded-md overflow-hidden">
                    <div className="relative w-full bg-black aspect-video">
                      <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover" />

                      <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 p-1 md:p-2 flex justify-between items-center">
                        <select
                          value={fileExtension}
                          onChange={(e) => setFileExtension(e.target.value)}
                          className="px-1 md:px-2 py-0.5 md:py-1 text-xs md:text-sm rounded bg-gray-800 text-white border border-gray-700"
                        >
                        ...
                        <button
                          ...
                          className="bg-red-600 text-white rounded-full w-8 h-8 md:w-12 md:h-12 flex items-center justify-center"
                        >
                          <div className="w-6 h-6 md:w-10 md:h-10 rounded-full border-2 border-white"></div>
                        </button>

                        <button
                          onClick={toggleCamera}
                          className="px-1 md:px-2 py-0.5 md:py-1 bg-gray-700 text-white rounded text-xs md:text-sm"
                        >
                          {t("cancel")}
                        </button>
                      </div>
            ...
            <button
              ...
              className="p-3 md:p-4 border-t w-full bg-blue-900 text-white px-4 py-2 rounded-b-md hover:bg-blue-950 cursor-pointer transition-colors text-sm md:text-base"
            >
      ...
      <div className="space-y-4 md:space-y-6 mt-4 md:mt-8">
        {meetings.map((meeting) => (
          ...
          >
            <div className="absolute top-0 left-3 md:left-6 px-2 md:px-4 py-0.5 md:py-1 bg-blue-950 text-white text-xs md:text-sm realfont rounded-b-lg shadow-sm transform transition-transform group-hover:translate-y-0.5">
              {meeting.date}
            </div>
            ...
            <div className="pt-6 md:pt-8 pb-3 md:pb-4 px-3 md:px-6 realfont">
              <div className="flex flex-col md:flex-row md:items-center justify-between">
                ...
                <div className="flex items-center justify-center md:justify-end gap-3 mt-3 md:mt-0">
                  <button
                    ...
                    className="flex items-center justify-center h-8 w-8 md:h-9 md:w-9 rounded-full bg-blue-100 text-blue-950 hover:bg-blue-200 transition-colors"
                  ...
                  <button
                    ...
                    className="flex items-center justify-center h-8 w-8 md:h-9 md:w-9 rounded-full bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
              ...
              <div className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
