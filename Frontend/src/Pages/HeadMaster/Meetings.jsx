...
const Meetings = () => {
  ...
  const fetchMeetings = async () => {
    try {
      ...
      const [meetingsResponse, countsResponse] = await Promise.all([
        ...
        axios.get(
          `http://localhost:5000/api/tharav/count?school_id=${SchoolId}`
        ),
      ]);
  ...
  const toggleModal = async () => {
    if (!isOpen) {
      ...
      // Start location detection in the background without blocking
      // This avoids the full skeleton loader while still getting location
      if (!isEditing) {
        detectLocation(false); // Pass false to not trigger the full loader
      }

      // Only perform validation check when adding a new meeting
      if (!isEditing) {
        const canCreateNewMeeting = await checkCanCreateNewMeeting();
        if (!canCreateNewMeeting) {
          // If validation fails, reset form and close
          resetForm();
          return;
        }
      }
  ...
  const checkCanCreateNewMeeting = async () => {
    ...
    try {
      ...
      for (const meeting of meetings) {
        ...
        if (!tharavRecords || tharavRecords.length === 0) {
          Swal.fire({
            ...
            html: `<div class="text-left">
              <p class="mb-2">${t("Meeting")} #${meeting.number} ${t(
              "has no tharav records"
            )}</p>
              <p class="text-sm text-gray-600">${t(
                "Please add at least one tharav record to this meeting before creating a new one"
              )}</p>
            </div>`,
        ...
        if (!allTharavsValid) {
          Swal.fire({
            ...
            html: `<div class="text-left">
              <p class="mb-2">${t("Meeting")} #${meeting.number} ${t(
              "has incomplete tharavs"
            )}</p>
              <p class="text-sm text-gray-600">${t(
                "Each tharav must be either completed (Active) or have its own remarks"
              )}</p>
              <p class="text-sm text-gray-600 mt-2">${t(
                "Number of incomplete tharavs"
              )}: ${invalidTharavs.length}</p>
            </div>`,
            confirmButtonText: t("OK"),
          });
          setIsCheckingMeeting(false);
      ...
      Swal.fire({
        ...
        title: t("Error"),
        text: t(
          "An error occurred while checking if a new meeting can be created."
        ),
      });
      setIsCheckingMeeting(false);
      return false;
  ...
  const detectLocation = (showFullLoader = true) => {
    console.log("detectLocation called");
    if (navigator.geolocation) {
      console.log("Geolocation is supported");

      // Only set loading to true if showFullLoader is true
      if (showFullLoader) {
        setLoading(true);
      }
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setLatitude(lat);
          setLongitude(lon);
          getAddressFromGoogle(lat, lon, showFullLoader);
        },
        (error) => {
    ...
    <>
      {loading ? (
        ...
        <div className="min-h-screen p-3 md:p-6 space-y-4 md:space-y-6">
          <div className="bg-blue-950 text-white  md:p-4 flex justify-between -mb-[1px] rounded-t-[14px] items-center sticky top-0 z-10   md:max-w-[913px] ml-[152px]  ">
            <h2 className="text-xl md:text-3xl font-bold realfont2">
              {t("smcMeetings")}
            </h2>
            <button
              onClick={toggleModal}
            ...
            >
              {isCheckingMeeting ? (
                <>
                  ...
                  <span className="text-sm sm:text-base realfont2">
                    {t("Checking...")}
                  </span>
                </>
              ) : (
          ...
          <div className="bg-white rounded-b-[14px] mx-auto w-[75%] max-w-6xl overflow-y-auto max-h-[70vh] shadow-md pb-[80px]">
            {isOpen && (
